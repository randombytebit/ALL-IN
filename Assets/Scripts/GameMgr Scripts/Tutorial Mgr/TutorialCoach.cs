...
public class TutorialCoachManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (displayText == null)
        {
            ...
            Transform tmpTransform = System.Array.Find(allObjects, t => t.name == "COACH_TMP");            
            if (tmpTransform != null)
            {
                displayText = tmpTransform.GetComponent<TextMeshProUGUI>();
                Debug.Log("COACH_TMP GameObject found and TextMeshProUGUI component assigned.");
            }
            else
            {
                Debug.LogError("COACH_TMP GameObject not found in the scene.");
            }
        }
